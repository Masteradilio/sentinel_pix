#!/usr/bin/env python
# -*- coding: utf-8 -*-
# EXP-013A-R2 — Statistical FP/TP Diagnostics
#
# Versão completa corrigida. Substitui o script anterior.
# Corrige erro de quantile em booleanos convertendo todas as séries numéricas
# para float antes dos testes e quantis.
...
DERIVED_LABEL_COLS = {
    ...
    "union_flagged",
    "intersection_flagged",
    "shadow_recovers_runtime_fn",
    "shadow_adds_fp_vs_runtime",
    "shadow_loses_runtime_tp",
    "shadow_removes_runtime_fp",
    "exp012d_pred",
    "r4_pred",
    "lgbm_r4_pred",
}
...
ID_LIKE_SUBSTRINGS = [
    "transaction_id", "cd_pix", "cpf", "cnpj", "customer_id",
    "counterparty_id", "session_id", "ip_address", "hash", "chave_pix"
]

PREFERRED_NUMERIC = [
    "lgbm_r4_score", "r4_score", "if_percentile", "if_raw",
    "se_score", "se_pattern_count", "se_has_critico", "se_max_pattern_score",
    "behavioral_score", "behavioral_risk_factor_count",
    "behavioral_has_velocity_factor", "behavioral_has_dormancy_factor",
    "behavioral_has_age_value_factor", "behavioral_max_precision",
    "vl_pix", "hour",
    ...
    "max_qtd_pix_dia_pagador_7d", "max_qtd_pix_dia_pagador_30d", "max_qtd_pix_dia_pagador_90d",
    ...
    "burst_daily_7d_flag", "topaz_risk_score", "topaz_transacao_rejeitada", "mbk_completeness_score",
]
...
def numeric_float_clean(series: pd.Series) -> pd.Series:
    return (
...
def cliff_delta_from_u(u: float, n_x: int, n_y: int) -> float:
    if n_x == 0 or n_y == 0:
        return np.nan
...
def odds_ratio_ci(a: float, b: float, c: float, d: float) -> tuple[float, float, float]:
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["shadow_exp012d_flagged", "runtime_flagged", "exp012d_pred", "r4_pred", "lgbm_r4_pred"]:
        if c in df.columns:
    ...
    if "shadow_exp012d_flagged" not in df.columns:
        if "exp012d_pred" in df.columns:
            ...
            df["shadow_exp012d_flagged"] = df["r4_pred"].astype(int)
        elif "lgbm_r4_pred" in df.columns:
            df["shadow_exp012d_flagged"] = df["lgbm_r4_pred"].astype(int)
        else:
            raise RuntimeError("Não encontrei shadow_exp012d_flagged, exp012d_pred, r4_pred ou lgbm_r4_pred.")
    ...
    if "shadow_recovers_runtime_fn" not in df.columns:
        df["shadow_recovers_runtime_fn"] = (
            (df["is_fraud"] == 1) & (df["runtime_flagged"] == 0) & (df["shadow_exp012d_flagged"] == 1)
        ).astype(int)

    if "shadow_adds_fp_vs_runtime" not in df.columns:
        df["shadow_adds_fp_vs_runtime"] = (
            (df["is_fraud"] == 0) & (df["runtime_flagged"] == 0) & (df["shadow_exp012d_flagged"] == 1)
        ).astype(int)
...
def prepare_group(df: pd.DataFrame, group_mode: str) -> tuple[pd.DataFrame, str, str]:
    work = df.copy()

    if group_mode == "shadow_tp_vs_fp":
        sub = work[work["shadow_exp012d_flagged"] == 1].copy()
        sub["group_y"] = sub["is_fraud"].astype(int)
        return sub, "TP_shadow", "FP_shadow"

    if group_mode == "recovered_fn_vs_added_fp":
    ...
    raise ValueError(f"group_mode inválido: {group_mode}")


def infer_numeric_columns(df: pd.DataFrame, max_cols: int | None = None) -> list[str]:
    cols = []
...
def infer_categorical_columns(df: pd.DataFrame, max_cardinality: int = 50) -> list[str]:
    cols = []
    ...
    for c in df.columns:
        ...
        if pd.api.types.is_object_dtype(df[c]) or pd.api.types.is_string_dtype(df[c]) or pd.api.types.is_bool_dtype(df[c]):
            ...
            nunique = numeric_float_clean(df[c]).nunique(dropna=True)
            if 1 < nunique <= 12:
                cols.append(c)
...
def run_numeric_tests(group_df: pd.DataFrame, numeric_cols: list[str], group_positive_name: str, group_negative_name: str) -> pd.DataFrame:
    rows = []
    ...
    for col in numeric_cols:
        ...
        x1 = numeric_float_clean(group_df.loc[y == 1, col])
        x0 = numeric_float_clean(group_df.loc[y == 0, col])

        if len(x1) < 5 or len(x0) < 5:
            continue
        if x1.nunique() <= 1 and x0.nunique() <= 1:
            continue
        ...
        try:
            ...
            valid[col] = pd.to_numeric(valid[col], errors="coerce").replace([np.inf, -np.inf], np.nan).astype(float)
            valid = valid.dropna()
            if valid["group_y"].nunique() == 2 and valid[col].nunique() > 1:
                ...
                if auc_raw >= 0.5:
                    ...
                    direction = "lower_in_tp"
        except Exception:
            pass

        rows.append({
            "feature": col,
            "group_positive": group_positive_name,
            "group_negative": group_negative_name,
            "n_pos_nonnull": int(len(x1)),
            "n_neg_nonnull": int(len(x0)),
            "missing_pos_rate": round(float(1 - len(x1) / max((y == 1).sum(), 1)), 6),
            "missing_neg_rate": round(float(1 - len(x0) / max((y == 0).sum(), 1)), 6),
            "mean_pos": float(x1.mean()),
            "mean_neg": float(x0.mean()),
            "median_pos": float(x1.median()),
            "median_neg": float(x0.median()),
            "p10_pos": float(x1.quantile(0.10)),
            "p10_neg": float(x0.quantile(0.10)),
            "p25_pos": float(x1.quantile(0.25)),
            "p25_neg": float(x0.quantile(0.25)),
            "p75_pos": float(x1.quantile(0.75)),
            "p75_neg": float(x0.quantile(0.75)),
            "p90_pos": float(x1.quantile(0.90)),
            "p90_neg": float(x0.quantile(0.90)),
            "mannwhitney_p": mw_p,
            "ks_stat": ks_stat,
            "ks_p": ks_p,
            "cliffs_delta": delta,
    ...
    out["robust_signal_score"] = (
        (out["auc_oriented"].fillna(0.5) - 0.5) * 2.0
        + out["cliffs_delta"].abs().fillna(0.0)
        + out["ks_stat"].fillna(0.0)
    )
    out["is_robust_signal"] = (
        (out["mannwhitney_q_bh"] <= 0.05)
        & (out["ks_q_bh"] <= 0.05)
        & (out["auc_oriented"].fillna(0.5) >= 0.60)
        & (out["cliffs_delta"].abs().fillna(0.0) >= 0.147)
    ).astype(int)

    return out.sort_values(["is_robust_signal", "robust_signal_score"], ascending=[False, False]).reset_index(drop=True)


def run_categorical_tests(group_df: pd.DataFrame, cat_cols: list[str], group_positive_name: str, group_negative_name: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    test_rows = []
    level_rows = []
    y = group_df["group_y"].astype(int)
    ...
    for col in cat_cols:
        ...
        test_rows.append({
            ...
            "group_negative": group_negative_name,
            "n": int(len(s2)),
            "n_levels": int(s2.nunique(dropna=False)),
            "chi2": chi2_stat,
            "chi2_p": chi_p,
            "cramers_v": cv,
        ...
        for level in s2.unique():
            ...
            level_rows.append({
                ...
                "fp_out_level": d,
                "n_level": a + b,
                "precision_in_level": level_precision,
                "base_precision": base_precision,
                "fraud_lift_vs_base": level_precision / max(base_precision, 1e-9),
                "tp_contribution": a / max(total_tp, 1),
                "fp_contribution": b / max(total_fp, 1),
                "odds_ratio": orv,
                "or_ci_low": lo,
                "or_ci_high": hi,
                "fisher_p": fisher_p,
    ...
    if not tests.empty:
        ...
        tests["is_robust_association"] = (
            (tests["chi2_q_bh"] <= 0.05) & (tests["cramers_v"].fillna(0.0) >= 0.10)
        ).astype(int)
...
def build_segments(group_df: pd.DataFrame, segment_sets: list[list[str]], min_segment_n: int, max_tp_loss: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    work = group_df.copy()
    ...
    for cols in segment_sets:
        ...
        tmp = pd.DataFrame({"segment_key": seg_key, "group_y": work["group_y"].astype(int).values})
        tmp["n"] = 1
        ...
        for _, r in agg.iterrows():
            ...
            row = {
                ...
                "fp": fp,
                "precision": precision,
                "fraud_lift_vs_base": precision / max(base_precision, 1e-9),
                "tp_contribution": tp / max(total_tp, 1),
                "fp_contribution": fp / max(total_fp, 1),
                "fp_minus_tp": fp - tp,
            }
...
def numeric_threshold_hypotheses(group_df: pd.DataFrame, numeric_tests: pd.DataFrame, max_features: int = 25) -> pd.DataFrame:
    ...
    features = numeric_tests.head(max_features)["feature"].tolist()
    quantiles = [0.01, 0.02, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]

    for feat in features:
        if feat not in work.columns:
            continue

        vals = pd.to_numeric(work[feat], errors="coerce").replace([np.inf, -np.inf], np.nan).astype(float)
        valid_vals = vals.dropna()
        if valid_vals.size < 20 or valid_vals.nunique() < 2:
            continue

        direction = numeric_tests.loc[numeric_tests["feature"] == feat, "direction"].iloc[0]
        qs = valid_vals.quantile(quantiles).drop_duplicates().tolist()
        ...
        for th in qs:
            ...
            tp_loss = int(((y == 1) & veto).sum())
            fp_removed = int(((y == 0) & veto).sum())
            ...
            rows.append({
                ...
                "direction": direction,
                "threshold": float(th),
                "tp_loss_if_veto": tp_loss,
                "fp_removed_if_veto": fp_removed,
                "recall_after_veto": (total_tp - tp_loss) / max(total_tp, 1),
                "fp_after_veto": total_fp - fp_removed,
                "fp_removed_per_tp_lost": fp_removed / max(tp_loss, 1),
                "source_auc_oriented": float(numeric_tests.loc[numeric_tests["feature"] == feat, "auc_oriented"].iloc[0]),
                "source_cliffs_delta": float(numeric_tests.loc[numeric_tests["feature"] == feat, "cliffs_delta"].iloc[0]),
                "source_mannwhitney_q_bh": float(numeric_tests.loc[numeric_tests["feature"] == feat, "mannwhitney_q_bh"].iloc[0]),
            })
...
def make_report(summary: dict[str, Any], numeric: pd.DataFrame, cats: pd.DataFrame, levels: pd.DataFrame, veto: pd.DataFrame, hypotheses: pd.DataFrame) -> str:
    lines = []
    lines.append("# EXP-013A — Statistical FP/TP Diagnostics")
    lines.append("")
    lines.append("## Amostra principal")
    for k in ["n_rows_input", "n_shadow_positive", "n_shadow_tp", "n_shadow_fp", "n_recovered_runtime_fn", "n_added_fp_vs_runtime"]:
        lines.append(f"- {k}: {summary.get(k)}")

    lines.append("")
    lines.append("## Top variáveis numéricas robustas")
    if numeric.empty:
        lines.append("Nenhuma variável numérica testável encontrada.")
    else:
    ...
    lines.append("")
    lines.append("## Top associações categóricas")
    if cats.empty:
        lines.append("Nenhuma variável categórica testável encontrada.")
    else:
        cols = ["feature", "n_levels", "chi2_q_bh", "cramers_v", "is_robust_association"]
        lines.append(cats[cols].head(20).to_markdown(index=False))

    lines.append("")
    lines.append("## Níveis com maior lift de fraude")
    if levels.empty:
        lines.append("Nenhum nível categórico relevante encontrado.")
    else:
    ...
    if veto.empty:
        lines.append("Nenhum segmento simples com baixo TP loss e alto FP removível foi encontrado pelos critérios atuais.")
    else:
    ...
    lines.append("")
    lines.append("## Hipóteses de filtros por threshold")
    if hypotheses.empty:
        lines.append("Nenhuma hipótese numérica gerada.")
    else:
    ...
    lines.append("Usar esses achados no EXP-013B — Statistical Policy Search.")
    return "\n".join(lines)
...
def main() -> None:
    ...
    numeric_cols = infer_numeric_columns(g_shadow, max_cols=args.max_numeric_cols)
    ...
    segments, veto = build_segments(
        g_shadow,
        segment_sets=SEGMENT_SETS,
        min_segment_n=args.min_segment_n,
        max_tp_loss=args.max_tp_loss_segment,
    )

    hypotheses = numeric_threshold_hypotheses(g_shadow, numeric, max_features=25)

    g_recovered, pos2_name, neg2_name = prepare_group(df, "recovered_fn_vs_added_fp")
    numeric2 = run_numeric_tests(g_recovered, numeric_cols, pos2_name, neg2_name)
    cats2, _ = run_categorical_tests(g_recovered, cat_cols, pos2_name, neg2_name)

    summary = {
        "experiment": "EXP-013A",
        "script_version": "R2_BOOL_SAFE",
        "status": "DONE",
        "input_path": str(input_path),
        "n_rows_input": int(len(df)),
        "n_shadow_positive": int(len(g_shadow)),
        "n_shadow_tp": int((g_shadow["group_y"] == 1).sum()),
        "n_shadow_fp": int((g_shadow["group_y"] == 0).sum()),
        "n_recovered_runtime_fn": int((g_recovered["group_y"] == 1).sum()) if not g_recovered.empty else 0,
        "n_added_fp_vs_runtime": int((g_recovered["group_y"] == 0).sum()) if not g_recovered.empty else 0,
        "n_numeric_features_tested": int(len(numeric)),
        "n_categorical_features_tested": int(len(cats)),
        "n_segment_rows": int(len(segments)),
        "n_veto_candidate_segments": int(len(veto)),
    ...
    dump_json({
        ...
        "segment_sets": SEGMENT_SETS,
        "derived_label_cols_excluded": sorted(DERIVED_LABEL_COLS),
        "id_like_substrings_excluded": ID_LIKE_SUBSTRINGS,
        "tests": {
            "numeric": ["Mann-Whitney U", "Kolmogorov-Smirnov", "Cliff's Delta", "univariate ROC-AUC", "Benjamini-Hochberg FDR"],
            "categorical": ["Chi-square", "Cramer's V", "Odds Ratio", "Fisher exact when applicable", "Benjamini-Hochberg FDR"],
            "segments": ["precision/lift/contribution", "veto candidate simulation"],
        },
    }, output_dir / "10_data_dictionary_used.json")

    report = make_report(summary, numeric, cats, levels, veto, hypotheses)
    (output_dir / "09_distribution_quality_report.md").write_text(report, encoding="utf-8")

    print("=" * 80)
    print("EXP-013A concluído")
    print("=" * 80)
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    print("\nArquivos principais:")
    for p in [
        output_dir / "00_run_summary.json",
        output_dir / "01_numeric_tests_shadow_tp_vs_fp.csv",
        output_dir / "02_categorical_tests_shadow_tp_vs_fp.csv",
        output_dir / "03_categorical_level_odds_shadow_tp_vs_fp.csv",
        output_dir / "04_segment_analysis_shadow_positives.csv",
        output_dir / "05_veto_candidate_segments.csv",
        output_dir / "06_feature_threshold_hypotheses.csv",
        output_dir / "09_distribution_quality_report.md",
    ]:
        print(f"  {p}")
