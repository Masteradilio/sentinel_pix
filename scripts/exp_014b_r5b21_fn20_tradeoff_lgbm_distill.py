#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""EXP-014B-R5B21 - FN20 trade-off and LGBM distillation of R5B16."""
...
EXPERIMENT = "EXP-014B-R5B21-FN20-TRADEOFF-LGBM-DISTILL"
...
TRADEOFF_ACTION_COL = "r5b21_fn20_tradeoff_decisao"
FN_BUDGET = 20
...
def candidate_record(df: pd.DataFrame, mask: pd.Series, rule_id: str, description: str, target_action: str) -> dict[str, Any]:
    ...
    return {
        ...
        "n_frauds": frauds,
        "n_normals": normals,
        "normal_rate": round(float(normals / max(len(selected), 1)), 8),
    }
...
def add_candidate(candidates: list[tuple[pd.Series, dict[str, Any]]], df: pd.DataFrame, mask: pd.Series, rule_id: str, desc: str, target: str) -> None:
    rec = candidate_record(df, mask, rule_id, desc, target)
    block_normal_rate = BASELINE["block"]["fp"] / (BASELINE["block"]["tp"] + BASELINE["block"]["fp"])
    if rec["n_normals"] > 0 and rec["n_frauds"] <= FN_BUDGET and rec["normal_rate"] > block_normal_rate:
        candidates.append((mask, rec))


def mine_tradeoff_candidates(df: pd.DataFrame) -> list[tuple[pd.Series, dict[str, Any]]]:
    ...
    add_candidate(candidates, df, block & layer.eq("APPROVE_TO_BLOCK"), "DEMOTE_LAYER_APPROVE_TO_BLOCK_TO_APROVAR", "Demove APPROVE_TO_BLOCK para APROVAR", "APROVAR")
    add_candidate(candidates, df, block & layer.eq("CONFIRM_TO_BLOCK"), "DEMOTE_LAYER_CONFIRM_TO_BLOCK_TO_CONFIRMAR", "Demove CONFIRM_TO_BLOCK para CONFIRMAR", "CONFIRMAR")

    for col in [c for c in CAT_COLS if c in df.columns]:
        series = df[col].fillna("<MISSING>").astype(str)
        for value in series[block].value_counts().index[:100]:
            mask = block & series.eq(value)
            add_candidate(candidates, df, mask, f"DEMOTE_CAT_{col}_{value}", f"{col} == {value}", "CONFIRMAR")

    for col_a, col_b in itertools.combinations([c for c in CAT_COLS if c in df.columns], 2):
        a = df[col_a].fillna("<MISSING>").astype(str)
        b = df[col_b].fillna("<MISSING>").astype(str)
        pairs = pd.Series(list(zip(a[block], b[block]))).value_counts().head(200).index
        for va, vb in pairs:
            mask = block & a.eq(va) & b.eq(vb)
            add_candidate(candidates, df, mask, f"DEMOTE_CAT2_{col_a}_{va}__{col_b}_{vb}", f"{col_a} == {va} AND {col_b} == {vb}", "CONFIRMAR")

    for col in [c for c in NUMERIC_COLS if c in df.columns]:
        values = pd.to_numeric(df.loc[block, col], errors="coerce")
        if not values.notna().any():
            continue
        thresholds = np.unique(np.nanquantile(values.dropna(), np.linspace(0.01, 0.75, 120)))
...
def select_tradeoff(df: pd.DataFrame, candidates: list[tuple[pd.Series, dict[str, Any]]]) -> list[tuple[pd.Series, dict[str, Any]]]:
    ...
    for mask, rec in candidates:
        ...
        if inc_normals <= 0 or frauds_used + inc_frauds > FN_BUDGET:
            continue
        inc_normal_rate = inc_normals / max(int(incremental.sum()), 1)
        block_normal_rate = BASELINE["block"]["fp"] / (BASELINE["block"]["tp"] + BASELINE["block"]["fp"])
        if inc_normal_rate <= block_normal_rate:
            continue
        selected |= incremental
        frauds_used += inc_frauds
        rule = dict(rec)
        ...
        if frauds_used >= FN_BUDGET:
            break
    return rules


def evaluate_tradeoff(df: pd.DataFrame, rules: list[tuple[pd.Series, dict[str, Any]]]) -> tuple[pd.DataFrame, dict[str, Any], list[dict[str, Any]]]:
    out = df.copy()
    ...
    block = out[TRADEOFF_ACTION_COL].eq("BLOQUEAR").astype(int)
    ...
    return out, summary, serializable_rules


def load_split(path: Path) -> pd.DataFrame:
    df = pd.read_csv(path, low_memory=False)
    contract = pd.read_csv(R5B18, low_memory=False)[[
        "transaction_id",
...
def feature_columns(df: pd.DataFrame) -> list[str]:
    return [c for c in df.columns if c not in DROP_COLS and c not in {BASE_ACTION_COL, "contract_intervention", "contract_block"} and df[c].nunique(dropna=True) > 1]
...
def encode_frames(frames: list[pd.DataFrame], cols: list[str]) -> tuple[list[pd.DataFrame], list[str]]:
    ...
    cats = {
        c: {v: i for i, v in enumerate(pd.concat([f[c].fillna("<MISSING>").astype(str) for f in frames], ignore_index=True).unique())}
        for c in cat_cols
    }
    encoded = []
    for frame in frames:
...
def threshold_search(y_true: pd.Series, score: np.ndarray, fn_budget: int) -> tuple[float, dict[str, Any]]:
    ...
    if not rows:
        thr = float(np.nanmin(score))
        return thr, metrics(y_true, pd.Series(score >= thr).astype(int))
    return max(rows, key=lambda item: (item[1]["precision"] + item[1]["f1"], -item[1]["fpr"]))


def train_distiller(target_col: str, train: pd.DataFrame, val: pd.DataFrame, holdout: pd.DataFrame, full: pd.DataFrame, x_train: pd.DataFrame, x_val: pd.DataFrame, x_holdout: pd.DataFrame, x_full: pd.DataFrame) -> dict[str, Any]:
    ...
    threshold, validation_fraud_metrics = threshold_search(
        val[LABEL_COL],
        validation_score,
        max(1, round(FN_BUDGET * int(val[LABEL_COL].sum()) / 1465)),
    )
    validation_contract_metrics = metrics(y_val, pd.Series(validation_score >= threshold).astype(int))
    out = {
        "target_col": target_col,
        "threshold": threshold,
        "validation_contract_metrics": validation_contract_metrics,
        "validation_fraud_metrics": validation_fraud_metrics,
        "scopes": {},
    }
...
def run_distillation() -> dict[str, Any]:
    ...
    intervention = train_distiller("contract_intervention", train, val, holdout, full, x_train, x_val, x_holdout, x_full)
    block = train_distiller("contract_block", train, val, holdout, full, x_train, x_val, x_holdout, x_full)
...
def main() -> None:
    ...
    candidates = mine_tradeoff_candidates(df)
    rules = select_tradeoff(df, candidates)
    tradeoff_df, tradeoff_summary, selected_rules = evaluate_tradeoff(df, rules)
    distill = run_distillation()
    target_gates = {
        "tradeoff_fn_outside_block_lte_20": tradeoff_summary["fn_outside_block"] <= FN_BUDGET,
        ...
        "distill_global_precision_recall_f1_gte_80pct": all(
            distill["ratios_to_r5b16"]["global"][k] >= 0.8 for k in ["precision_ratio", "recall_ratio", "f1_ratio"]
        ),
        "distill_block_precision_recall_f1_gte_80pct": all(
            distill["ratios_to_r5b16"]["block"][k] >= 0.8 for k in ["precision_ratio", "recall_ratio", "f1_ratio"]
        ),
    }
    status = "PASS_R5B21_FN20_TRADEOFF_LGBM_DISTILL" if all(target_gates.values()) else "FAIL_R5B21_FN20_TRADEOFF_LGBM_DISTILL"
    summary = {
    ...
    tradeoff_df[["transaction_id", LABEL_COL, BASE_ACTION_COL, TRADEOFF_ACTION_COL]].to_csv(OUT_DIR / "02_tradeoff_predictions.csv", index=False)
