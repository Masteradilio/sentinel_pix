...
# -*- coding: utf-8 -*-
"""
EXP-014B-R3Q — Residual FP-only Boundary Probe sobre R3P-FROZEN

Objetivo:
  Testar a fronteira residual de falsos positivos após o R3P-FROZEN, preservando integralmente:
    TP=1465
    FN=0
    recall=100%
...
Princípios:
  ...
  - não chamar runtime;
  - minerar apenas vetos sobre alertas atuais do R3P-FROZEN;
  - aceitar somente regras com TP_loss=0;
  - exigir estabilidade temporal mínima e suporte em VALIDATION/HOLDOUT para reduzir risco de overfitting.

Uso recomendado:
  python scripts/exp_014b_r3q_residual_fp_only_boundary_probe.py

Uso um pouco mais exploratório, ainda conservador:
  python scripts/exp_014b_r3q_residual_fp_only_boundary_probe.py --max-rules 15 --min-fp-removed 12 --max-combo-size 3 --top-groups-per-combo 30

Saídas:
  resultados/experimentos/EXP-014B-R3Q/
    00_run_summary.json
...
DEFAULT_INPUT = PROJECT_ROOT / "resultados" / "experimentos" / "EXP-014B-R3P-FROZEN" / "08_predictions_frozen.csv"
DEFAULT_ARTIFACT = PROJECT_ROOT / "resultados" / "experimentos" / "EXP-014B-R3P-FROZEN" / "07_policy_artifact_frozen.json"
DEFAULT_OUT = PROJECT_ROOT / "resultados" / "experimentos" / "EXP-014B-R3Q"

BASE_COL = "exp014b_r3p_frozen_pred"
FINAL_COL = "exp014b_r3q_recommended_pred"

EXPECTED_BASE = {
    "tp": 1465,
    "fp": 4221,
    "fn": 0,
    "precision": 0.25765037,
    "recall": 1.0,
    "fpr": 0.0375604,
    "wilson_low_min": 0.99,
}
...
def normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in [BASE_COL, FINAL_COL, "exp014b_r3q_recommended_pred", "exp014b_r3p_frozen_pred", "exp014b_r3o_frozen_pred", "exp014b_r3n_frozen_pred"]:
        if c in df.columns:
...
def stability_counts(df: pd.DataFrame, mask: np.ndarray) -> tuple[int, int, bool, bool]:
    ...
    split_upper = tmp["temporal_split"].astype(str).str.upper()
    has_nontrain = bool((split_upper != "TRAIN").any())
    has_val_or_holdout = bool(split_upper.isin(["VALIDATION", "HOLDOUT", "VAL", "VALID", "TEST"]).any())
    return n_splits, n_months, has_nontrain, has_val_or_holdout
...
def candidate_ok(
    *,
    ...
    if require_nontrain_support and not has_nontrain:
        return False
    if require_holdout_or_validation_support and not has_val_or_holdout:
        return False
...
def add_candidate(
    ...
    *,
    ...
    n_splits, n_months, has_nontrain, has_val_or_holdout = stability_counts(df, mask)
    ...
    if not candidate_ok(
        ...
        has_val_or_holdout=has_val_or_holdout,
        ...
        require_holdout_or_validation_support=require_holdout_or_validation_support,
    ):
        return

    out.append(Candidate(
        rule_id=f"r3q_fp_{len(out):05d}",
        family=family,
        ...
        has_validation_or_holdout_support=has_val_or_holdout,
        combo_size=combo_size,
        params=params,
    ))
    ...
    for r in range(1, max_combo_size + 1):
        for combo in itertools.combinations(cols, r):
            ...
            if r >= 2 and not any(c in combo for c in important + bins):
                continue
            # Boundary probe dirigido: após R3P, os ganhos residuais vieram de
            # lgbm_bin + qtd_rec_bin. Para evitar grid caro, combos de 3 precisam
            # manter conexão explícita com sinais de score/recebedor.
            if r == 3:
                required_anchor = {"lgbm_bin", "qtd_rec_bin", "score_bin", "if_bin", "ratio_bin", "vl_bin"}
                if not ("lgbm_bin" in combo or "score_bin" in combo or "if_bin" in combo):
                    continue
                if len(required_anchor.intersection(combo)) < 2:
                    continue
            ...
            group_rows = []
            split_series = df["temporal_split"].astype(str).str.upper()
            month_series = df["event_month"].astype(str)
            for key, rel in sub.groupby(combo, dropna=False).indices.items():
                ...
                # Fast path: all rows are alerted by construction; avoid building full masks
                # until the candidate passes TP0/stability gates.
                tp_loss = int(y[rows].sum())
                fp_removed = int(len(rows) - tp_loss)
                if tp_loss != 0 or fp_removed < min_fp_removed:
                    continue

                fp_rows = rows[y[rows] == 0]
                if len(fp_rows) == 0:
                    continue
                split_vals = split_series.iloc[fp_rows]
                month_vals = month_series.iloc[fp_rows]
                n_splits = int(split_vals.nunique())
                n_months = int(month_vals.nunique())
                has_nontrain = bool((split_vals != "TRAIN").any())
                has_val_or_holdout = bool(split_vals.isin(["VALIDATION", "HOLDOUT", "VAL", "VALID", "TEST"]).any())
...
def candidate_dicts(cands: list[Candidate]) -> list[dict[str, Any]]:
    return [{
        ...
        "has_validation_or_holdout_support": bool(c.has_validation_or_holdout_support),
    ...
    for depth in range(1, max_rules + 1):
        ...
        for cand in pool:
            ...
            if not candidate_ok(
                ...
                has_val_or_holdout=marginal.has_validation_or_holdout_support,
...
def make_report(summary: dict[str, Any], base_val: dict[str, Any], frontier: pd.DataFrame, selected: pd.DataFrame, stability: pd.DataFrame, segments: pd.DataFrame) -> str:
    lines = [
        "# EXP-014B-R3Q — Residual FP-only Boundary Probe",
        "",
        "## Resultado executivo",
        f"- Status: `{summary['objective_status']}`",
        f"- All pass: `{summary['all_pass']}`",
        f"- Base R3P-FROZEN: `{summary['base_r3p_frozen_metrics']}`",
        f"- Métricas recomendadas R3Q: `{summary['recommended_metrics']}`",
        f"- FP removidos vs R3P: `{summary['fp_removed_vs_r3p']}`",
        f"- TP loss vs R3P: `{summary['tp_loss_vs_r3p']}`",
        f"- FN delta vs R3P: `{summary['fn_delta_vs_r3p']}`",
        "",
        "## Validação da base",
        "```json",
    ...
    if stability.empty:
        ...
        show = ["rule_id", "fp_removed", "tp_loss", "combo_size", "n_temporal_splits_with_fp_removed", "n_months_with_fp_removed", "has_nontrain_support", "has_validation_or_holdout_support", "description"]
    ...
    if summary["all_pass"] and summary["fp_removed_vs_r3p"] > 0:
        lines.append("R3Q gerou candidato FP-only de boundary probe superior ao R3P-FROZEN. Próximo passo: EXP-014B-R3Q-FROZEN, sem nova mineração.")
    else:
        lines.append("R3Q não encontrou ganho conservador suficiente. Manter R3P-FROZEN como benchmark principal e iniciar hardening/produção shadow.")
    return "\n".join(lines)
...
def main() -> None:
    ...
    parser.add_argument("--min-fp-removed", type=int, default=8)
    ...
    parser.add_argument("--require-holdout-or-validation-support", action="store_true", default=True)
    parser.add_argument("--no-require-holdout-or-validation-support", dest="require_holdout_or_validation_support", action="store_false")
    ...
    log("=" * 80)
    log("EXP-014B-R3Q — Residual FP-only Boundary Probe")
    log("=" * 80)
    ...
    base_validation = {
        ...
        "status": "PASS_R3P_FROZEN_BASE_VALIDATED" if base_metrics["tp"] == EXPECTED_BASE["tp"] and base_metrics["fp"] == EXPECTED_BASE["fp"] and base_metrics["fn"] == EXPECTED_BASE["fn"] and wl_base >= EXPECTED_BASE["wilson_low_min"] else "FAIL_R3P_FROZEN_BASE_DIVERGENCE",
    }
    dump_json(base_validation, out / "02_base_validation.json")

    log(f"Base R3P-FROZEN metrics: {base_metrics}")
    ...
    objective_status = "DONE_R3P_FROZEN_BASE_VALIDATED" if base_validation["all_pass"] else "DONE_R3P_FROZEN_BASE_NOT_VALIDATED"
    objective_status += "_FP_ONLY_REDUCED" if fp_removed > 0 else "_FP_ONLY_NO_GAIN"
    if fp_removed >= 50:
        objective_status += "_BOUNDARY_GAIN_RELEVANT"
    elif fp_removed >= 10:
        objective_status += "_BOUNDARY_MICRO_GAIN"
    elif fp_removed > 0:
        objective_status += "_BOUNDARY_TINY_GAIN"
    else:
        objective_status += "_BOUNDARY_NO_GAIN"
    ...
    artifact_out = {
        "experiment": "EXP-014B-R3Q",
        "policy_name": "r3q_residual_fp_only_conservative_reducer",
        "objective_status": objective_status,
        "source_artifact": str(artifact_path),
        "input_path": str(input_path),
        "base_col": BASE_COL,
        "final_pred_col": FINAL_COL,
        "base_r3p_frozen_metrics": base_metrics,
        "recommended_metrics": final_metrics,
        "fp_removed_vs_r3p": int(fp_removed),
        "tp_loss_vs_r3p": int(tp_loss),
        "fn_delta_vs_r3p": int(fn_delta),
        "wilson_low": wl,
        "wilson_high": wh,
        ...
        "notes": [
            ...
            "If this probe finds >=50 FP reduction, run EXP-014B-R3Q-FROZEN. If gain is micro, decide whether to freeze or declare boundary.",
        ],
    }
    ...
    (out / "09_exp014b_r3q_report.md").write_text(report, encoding="utf-8")
