...
# -*- coding: utf-8 -*-
"""
EXP-014B-R3M — Consolidate R3L-FROZEN + Residual FN v2 Optimizer

Objetivo:
  Consolidar o excelente resultado do EXP-014B-R3L e, na mesma execução,
  avançar a estratégia FN First / FP Second.

Parte A — Consolidação congelada do R3L:
  Reaplica o artifact recomendado do R3L sobre a base R3K, sem nova mineração,
  e valida o candidato:
...
Parte B — Próximo avanço:
  Usa o R3L congelado como nova base e tenta reduzir os 29 FNs residuais com:
    1. headroom TP0 adicional sobre alertas atuais;
    2. reuso das bibliotecas R3L e R3I de rescues;
    3. novos candidatos gerados diretamente dos 29 FNs residuais;
    4. re-tightening curto apenas sobre alertas adicionados;
    5. seleção de candidato se FN cair mantendo FP preferencialmente <= 5000.

Uso recomendado:
  python scripts/exp_014b_r3m_consolidate_r3l_and_residual_fn_v2.py --fp-caps 5000,5050,5100 --preferred-fp-cap 5000 --max-seconds-headroom 120 --max-seconds-retighten 90

Execução mais restrita:
  python scripts/exp_014b_r3m_consolidate_r3l_and_residual_fn_v2.py --fp-caps 5000 --max-new-combo-size 3 --max-seconds-headroom 90 --max-seconds-retighten 60

Saídas:
  resultados/experimentos/EXP-014B-R3M/
    00_run_summary.json
    01_input_contract.json
    02_r3l_frozen_validation.json
    03_r3l_frozen_metrics.csv
    04_r3l_frozen_rule_impact.csv
    05_r3m_base_metrics.csv
    06_r3m_headroom_candidates.csv
    07_r3m_headroom_selected_rules.csv
    08_r3m_headroom_metrics.csv
    09_r3m_rescue_candidates.csv
    10_r3m_scenarios_before_retightening.csv
    11_r3m_scenario_metrics_after_retightening.csv
    12_r3m_selected_rules_by_scenario.csv
    13_policy_artifact_recommended.json
    14_predictions_recommended.csv
    15_exp014b_r3m_report.md
"""
...
DEFAULT_INPUT = PROJECT_ROOT / "resultados" / "experimentos" / "EXP-014B-R3K" / "10_predictions_recommended.csv"
DEFAULT_R3L_ARTIFACT = PROJECT_ROOT / "resultados" / "experimentos" / "EXP-014B-R3L" / "11_policy_artifact_recommended.json"
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "resultados" / "experimentos" / "EXP-014B-R3M"

R3K_BASE_COL = "exp014b_r3k_recommended_pred"
R3L_FROZEN_COL = "exp014b_r3l_frozen_pred"
R3M_FINAL_COL = "exp014b_r3m_recommended_pred"

EXPECTED_R3L = {
    "scenario": "r3l_cap_5100",
    "tp": 1436,
    "fp": 4921,
    "fn": 29,
    "headroom_fp_removed": 197,
    "rescue_fn_recovered": 11,
    "rescue_fp_added": 321,
    "retightening_fp_removed": 169,
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in [R3K_BASE_COL, R3L_FROZEN_COL, R3M_FINAL_COL]:
        if c in df.columns:
...
def rule_mask(df: pd.DataFrame, current_pred: np.ndarray, params: dict[str, Any], mode: str, scope_mask: np.ndarray | None = None) -> np.ndarray:
    """
    mode:
      - headroom: current alerts -> set 0
      - rescue: current non-alerts -> set 1
      - retighten: current alerts -> set 0
    """
...
def validate_r3l_frozen(df: pd.DataFrame, artifact: dict[str, Any], rescue_df: pd.DataFrame, output_dir: Path) -> tuple[np.ndarray, dict[str, Any]]:
    y = df["is_fraud"].to_numpy(dtype=int)
    scenario_name = artifact.get("recommended_scenario") or EXPECTED_R3L["scenario"]
    scenario = artifact["scenario_artifacts"][scenario_name]
    base_pred = df[R3K_BASE_COL].to_numpy(dtype=int)
    ...
    all_impact.to_csv(output_dir / "04_r3l_frozen_rule_impact.csv", index=False)

    metrics_df = pd.DataFrame([
        {"policy_name": "R3K_BASE", **base_metrics},
        {"policy_name": "R3L_AFTER_HEADROOM", **head_metrics},
        {"policy_name": "R3L_AFTER_RESCUE_BEFORE_RETIGHTEN", **rescue_metrics},
        {"policy_name": "EXP014B_R3L_FROZEN_FINAL", **final_metrics},
    ])
    metrics_df.to_csv(output_dir / "03_r3l_frozen_metrics.csv", index=False)
    ...
    expected_metrics_match = (
        final_metrics["tp"] == EXPECTED_R3L["tp"]
        and final_metrics["fp"] == EXPECTED_R3L["fp"]
        and final_metrics["fn"] == EXPECTED_R3L["fn"]
    )
    expected_phases_match = (
        int(headroom_fp_removed) == EXPECTED_R3L["headroom_fp_removed"]
        and int(rescue_fn_recovered) == EXPECTED_R3L["rescue_fn_recovered"]
        and int(rescue_fp_added) == EXPECTED_R3L["rescue_fp_added"]
        and int(retightening_fp_removed) == EXPECTED_R3L["retightening_fp_removed"]
        and int(retightening_tp_loss) == EXPECTED_R3L["retightening_tp_loss"]
    )
    wilson_pass = wl >= EXPECTED_R3L["wilson_low_min"]
    ...
    validation = {
        ...
        "status": "PASS_R3L_FROZEN_VALIDATED" if all_pass else "FAIL_R3L_FROZEN_DIVERGENCE",
    }
    dump_json(validation, output_dir / "02_r3l_frozen_validation.json")
...
def greedy_select_tp0(cands: list[Candidate], pred: np.ndarray, y: np.ndarray, max_rules: int, max_seconds: int) -> tuple[np.ndarray, list[Candidate]]:
    ...
    for _ in range(max_rules):
        ...
        for i, c in enumerate(cands[:1200]):
            if i in used:
                continue
...
def add_rescue_candidate(rows, df, pred, mask, family, desc, params, min_fn_recovered, max_fp_added):
    ...
    if fn_recovered < min_fn_recovered or fp_added > max_fp_added:
        return
    rows.append({
        "candidate_id": f"r3m_rescue_{len(rows):05d}",
        "family": family,
        "description": desc,
        "fn_recovered": fn_recovered,
        "fp_added": fp_added,
...
def build_rescue_candidates(df, pred, rescue_libraries, min_fn_recovered, max_fp_added, max_new_combo_size, top_groups_per_combo):
    ...
    for c in NUMERIC_COLS:
        ...
        if len(fn_vals) == 0:
            continue
        try:
            cuts = sorted(set(float(x) for x in np.quantile(fn_vals, [0, 0.05, 0.10, 0.25, 0.50, 0.75, 0.90]) if np.isfinite(x)))
        except Exception:
            cuts = []
        for cut in cuts:
            for direction in ["ge", "le"]:
                ...
                add_rescue_candidate(
                    rows, df, pred, mask,
                    "new_numeric_residual_fn_rescue",
                    desc,
                    {"type": "numeric_threshold_rescue", "col": c, "direction": direction, "cut": cut},
                    min_fn_recovered,
                    max_fp_added,
                )

    # New combo candidates.
    feat = feature_frame(df)
    cols = list(feat.columns)
    ...
    for r in range(1, max_new_combo_size + 1):
        for combo in itertools.combinations(cols, r):
            combo = list(combo)
            if r == 1 and combo[0] not in ["lgbm_bin", "if_bin", "score_bin", "ds_tipo_chave_norm", "value_band", "ratio_bin", "qtd_rec_bin"]:
                continue
            sub = feat.iloc[idx][combo]
            if sub.empty:
                continue
            ...
            for key, rel_idxs in sub.groupby(combo, dropna=False).indices.items():
                ...
                if fn_recovered >= min_fn_recovered and fp_added <= max_fp_added:
                    groups.append((fp_added / max(fn_recovered, 1), -fn_recovered, key, mask))
            groups.sort()
            for _, _, key, mask in groups[:top_groups_per_combo]:
                ...
                add_rescue_candidate(
                    rows, df, pred, mask,
                    "new_combo_residual_fn_rescue",
                    desc,
                    {"type": "combo_rescue", "combo_cols": combo, "combo_values": vals},
                    min_fn_recovered,
                    max_fp_added,
                )
    ...
    out["candidate_id"] = [f"r3m_rescue_{i:05d}" for i in range(len(out))]
    return out
...
def greedy_rescue_for_caps(df, base_pred, rescue_candidates, fp_caps):
    ...
    rows = []
    pred_by_scenario = {}
    candidates = rescue_candidates.head(1800).to_dict(orient="records") if not rescue_candidates.empty else []
    ...
    for cap in fp_caps:
        ...
        pred_by_scenario[f"r3m_cap_{cap}"] = current
...
def run_retighten(df, headroom_pred, scenario_pred, min_fp_removed, max_combo_size, top_groups_per_combo, max_rules, max_seconds):
    ...
    # Mine TP0 vetoes only on added alerts by creating temporary pred = scenario and scope through mask intersection.
    cands = []
    # Numeric.
    for c in NUMERIC_COLS:
        if c not in df.columns:
    ...
    for r in range(1, max_combo_size + 1):
        for combo in itertools.combinations(cols, r):
            ...
            for key, rel_idxs in sub.groupby(combo, dropna=False).indices.items():
                ...
                fp_removed = int(((y == 0) & mask).sum())
                if tp_loss == 0 and fp_removed >= min_fp_removed:
                    groups.append((-fp_removed, key, mask, fp_removed))
            groups.sort()
            for _, key, mask, fp_removed in groups[:top_groups_per_combo]:
                vals = key if isinstance(key, tuple) else (key,)
                vals = [str(v) for v in vals]
...
def make_report(summary, validation, base_metrics, headroom_metrics, scenario_df, after_df, head_rules_df, selected_rules_df):
    lines = []
    lines.append("# EXP-014B-R3M — Consolidate R3L-FROZEN + Residual FN v2")
    lines.append("")
    lines.append("## Parte A — Consolidação R3L")
    lines.append(f"- Status R3L frozen: `{validation['status']}`")
    lines.append(f"- Métricas R3L frozen: `{validation['final_metrics']}`")
    lines.append("")
    lines.append("## Parte B — Próximo avanço")
    ...
    lines.append("")
    lines.append("## Base e headroom v2")
    lines.append(f"- Base R3L frozen: `{base_metrics}`")
    lines.append(f"- Após headroom v2: `{headroom_metrics}`")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Headroom v2 selecionado")
    if head_rules_df.empty:
        lines.append("Nenhuma regra de headroom v2 selecionada.")
    else:
    ...
    lines.append("Se o R3M reduzir FN abaixo de 29 mantendo FP dentro do cap, executar validação congelada. Se não houver ganho material, consolidar R3L como benchmark principal e avançar para hard-negative mining/segundo estágio/produção.")
    return "\n".join(lines)
...
def main() -> None:
    ...
    parser.add_argument("--r3l-artifact", default=str(DEFAULT_R3L_ARTIFACT))
    ...
    parser.add_argument("--fp-caps", default="5000,5050,5100")
    ...
    parser.add_argument("--max-fp-added-candidate", type=int, default=500)
    parser.add_argument("--min-fn-recovered", type=int, default=1)
    parser.add_argument("--max-new-combo-size", type=int, default=3)
    ...
    artifact_path = Path(args.r3l_artifact)
    ...
    log("=" * 80)
    log("EXP-014B-R3M — Consolidate R3L-FROZEN + Residual FN v2")
    log("=" * 80)

    for p, name in [(input_path, "input"), (artifact_path, "r3l_artifact"), (r3l_rescues_path, "r3l_rescues")]:
        if not p.exists():
    ...
    r3l_rescues = pd.read_csv(r3l_rescues_path)
    r3i_rescues = pd.read_csv(r3i_rescues_path) if r3i_rescues_path.exists() else pd.DataFrame()

    missing = []
    if "is_fraud" not in df.columns:
        missing.append("is_fraud")
    if R3K_BASE_COL not in df.columns:
        missing.append(R3K_BASE_COL)
    if "params_json" not in r3l_rescues.columns:
        missing.append("r3l_rescues.params_json")
    contract = {
        "n_rows": int(len(df)),
        "n_frauds": int(df["is_fraud"].sum()) if "is_fraud" in df.columns else None,
        "r3k_base_col": R3K_BASE_COL,
        "r3l_artifact_path": str(artifact_path),
    ...
    log("[A] Validando R3L congelado...")
    r3l_pred, validation = validate_r3l_frozen(df, artifact, r3l_rescues, output_dir)
    df[R3L_FROZEN_COL] = r3l_pred.astype(int)

    if not validation["all_pass"]:
        log("AVISO: R3L frozen não validou 100%. O experimento seguirá apenas se você revisar os artefatos.")
        # Do not abort automatically; all details are saved.

    # B) New advance from R3L frozen.
    base_pred = r3l_pred.copy()
    base_metrics = compute_metrics(y, base_pred)
    pd.DataFrame([{"policy_name": "R3L_FROZEN_BASE_FOR_R3M", **base_metrics}]).to_csv(output_dir / "05_r3m_base_metrics.csv", index=False)
    log(f"Base R3M metrics: {base_metrics}")

    log("[B1] Minerando headroom v2 TP0...")
    head_cands = mine_tp0_veto_candidates(
        ...
        prefix_family="headv2",
    )
    candidates_df(head_cands).to_csv(output_dir / "06_r3m_headroom_candidates.csv", index=False)
    ...
    head_rules_df.to_csv(output_dir / "07_r3m_headroom_selected_rules.csv", index=False)
    head_metrics = compute_metrics(y, head_pred)
    pd.DataFrame([{"policy_name": "R3M_HEADROOM_V2_BASE", **head_metrics}]).to_csv(output_dir / "08_r3m_headroom_metrics.csv", index=False)
    headroom_fp_removed = base_metrics["fp"] - head_metrics["fp"]
    log(f"Headroom v2 metrics: {head_metrics}")

    log("[B2] Gerando rescues residuais sobre FNs pós-R3L...")
    rescue_candidates = build_rescue_candidates(
        df=df,
        pred=head_pred,
        rescue_libraries=[("r3l", r3l_rescues), ("r3i", r3i_rescues)],
        min_fn_recovered=args.min_fn_recovered,
        max_fp_added=args.max_fp_added_candidate,
        max_new_combo_size=args.max_new_combo_size,
        top_groups_per_combo=args.top_groups_per_combo,
    )
    rescue_candidates.to_csv(output_dir / "09_r3m_rescue_candidates.csv", index=False)
    log(f"Rescue candidates R3M: {len(rescue_candidates)}")
    ...
    scenario_df.to_csv(output_dir / "10_r3m_scenarios_before_retightening.csv", index=False)
    ...
    after_df.to_csv(output_dir / "11_r3m_scenario_metrics_after_retightening.csv", index=False)

    selected_rules_df = pd.concat(all_selected, ignore_index=True) if all_selected else pd.DataFrame()
    selected_rules_df.to_csv(output_dir / "12_r3m_selected_rules_by_scenario.csv", index=False)

    recommended_scenario = select_recommended(after_df, args.preferred_fp_cap)
    rec_scenario_artifact = scenario_artifacts[recommended_scenario]
    # Reconstruct recommended final prediction for output.
    ...
    df[R3M_FINAL_COL] = rec_final_pred.astype(int)
    df["exp014b_r3m_recommended_scenario"] = recommended_scenario

    objective_status = "DONE_R3L_FROZEN_CONSOLIDATED"
    objective_status += "_R3M_FN_IMPROVED_VS_R3L" if rec_metrics["fn"] < base_metrics["fn"] else "_R3M_FN_NOT_IMPROVED_VS_R3L"
    objective_status += "_FP_WITHIN_PREFERRED_CAP" if rec_metrics["fp"] <= args.preferred_fp_cap else "_FP_ABOVE_PREFERRED_CAP"
    objective_status += "_R3L_FROZEN_PASS" if validation["all_pass"] else "_R3L_FROZEN_NOT_PASS"

    artifact_out = {
        "experiment": "EXP-014B-R3M",
        "policy_name": "consolidate_r3l_and_residual_fn_v2",
        "objective_status": objective_status,
        "r3l_frozen_validation": validation,
        "base_r3l_frozen_metrics": base_metrics,
        "headroom_v2_metrics": head_metrics,
        ...
        "notes": [
            "Part A consolidates R3L-FROZEN without new mining.",
            "Part B attempts one short FN-first/FP-second microevolution from R3L.",
            "If recommended_metrics improves FN below 29 and keeps FP within cap, run R3M-FROZEN next.",
            "If not, consolidate R3L as current benchmark and move to hard-negative mining/second stage/production preparation."
        ],
    }
    dump_json(artifact_out, output_dir / "13_policy_artifact_recommended.json")

    if not args.no_write_predictions:
        df.to_csv(output_dir / "14_predictions_recommended.csv", index=False)
    ...
    summary = {
        ...
        "r3l_frozen_validation_status": validation["status"],
        "r3l_frozen_metrics": validation["final_metrics"],
        "base_r3l_frozen_metrics": base_metrics,
        "headroom_v2_metrics": head_metrics,
        "headroom_v2_fp_removed": int(headroom_fp_removed),
    ...
    (output_dir / "15_exp014b_r3m_report.md").write_text(report, encoding="utf-8")
    ...
    for p in [
        ...
        output_dir / "02_r3l_frozen_validation.json",
        output_dir / "03_r3l_frozen_metrics.csv",
        output_dir / "04_r3l_frozen_rule_impact.csv",
        output_dir / "09_r3m_rescue_candidates.csv",
        output_dir / "11_r3m_scenario_metrics_after_retightening.csv",
        output_dir / "12_r3m_selected_rules_by_scenario.csv",
        output_dir / "13_policy_artifact_recommended.json",
        output_dir / "15_exp014b_r3m_report.md",
    ]:
        log(f"  {p}")
