# -*- coding: utf-8 -*-
"""
EXP-014B-R4G-FAST-FROZEN — Frozen validation do R4G-FAST.

Reaplica o artifact recomendado do EXP-014B-R4G-FAST sobre o baseline R4F-FROZEN
e valida replay exato:
  - métricas de intervenção;
  - métricas de BLOQUEAR;
  - predições recomendadas, se EXP-014B-R4G-FAST/11_predictions_recommended.csv existir.
...
EXPERIMENT = "EXP-014B-R4G-FAST-FROZEN"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--r4g-predictions", default=None)
...
def defaults() -> tuple[Path, Path, Path, Path]:
    root = Path.cwd()
    return (
        root / "resultados" / "experimentos" / "EXP-014B-R4F-FROZEN" / "06_predictions_frozen.csv",
        root / "resultados" / "experimentos" / "EXP-014B-R4G-FAST" / "10_policy_artifact_recommended.json",
        root / "resultados" / "experimentos" / "EXP-014B-R4G-FAST" / "11_predictions_recommended.csv",
...
def mask_from_description(df: pd.DataFrame, desc: str) -> pd.Series:
    prefixes = [
        "Mover APROVAR para CONFIRMAR R4G_FAST com ",
        "Mover BLOQUEAR para CONFIRMAR R4G_FAST com ",
    ]
...
def main() -> None:
    ...
    if not artifact_path.exists():
        raise FileNotFoundError(f"Artifact R4G-FAST não encontrado: {artifact_path}")
    ...
    ref_path = Path(args.r4g_predictions) if args.r4g_predictions else ref_default
    ...
    # Replay follows R4G-FAST phase order:
    # A) APROVAR -> CONFIRMAR
    approve_to_confirm = pd.Series(False, index=df.index)
    for rule in artifact.get("selected_approve_to_confirm_rules", []):
        approve_to_confirm |= base_action.eq("APROVAR") & mask_from_description(df, str(rule["description"]))

    action_after_a = base_action.copy()
    action_after_a.loc[approve_to_confirm] = "CONFIRMAR"

    # B) BLOQUEAR -> CONFIRMAR
    ...
    df["exp014b_r4g_fast_frozen_approve_to_confirm"] = approve_to_confirm.astype(int)
    df["exp014b_r4g_fast_frozen_block_to_confirm"] = block_to_confirm.astype(int)
    df["r4g_fast_frozen_decisao_recommended"] = final_action
    df["exp014b_r4g_fast_frozen_intervention_pred"] = intervention_from_action(final_action)
    df["exp014b_r4g_fast_frozen_block_pred"] = block_from_action(final_action)

    frozen_intervention = metrics(df[label_col], df["exp014b_r4g_fast_frozen_intervention_pred"])
    frozen_block = metrics(df[label_col], df["exp014b_r4g_fast_frozen_block_pred"])
    ...
    if ref_path.exists():
        ...
        src_action = artifact.get("final_action_col", "r4g_fast_decisao_recommended")
        src_intervention = artifact.get("intervention_pred_col", "exp014b_r4g_fast_intervention_pred")
        src_block = artifact.get("block_pred_col", "exp014b_r4g_fast_block_pred")

        if src_action in ref.columns:
            mismatch |= actions(ref[src_action]).ne(actions(df["r4g_fast_frozen_decisao_recommended"]))
        if src_intervention in ref.columns:
            mismatch |= ints(ref[src_intervention]).ne(ints(df["exp014b_r4g_fast_frozen_intervention_pred"]))
        if src_block in ref.columns:
            mismatch |= ints(ref[src_block]).ne(ints(df["exp014b_r4g_fast_frozen_block_pred"]))
    ...
    status = (
        "PASS_R4G_FAST_FROZEN_VALIDATED_REPLAY_OK"
        if intervention_ok and block_ok and n_mismatch == 0
        else "FAIL_R4G_FAST_FROZEN_VALIDATION_MISMATCH"
    )

    by_action = action_table(df, label_col, "r4g_fast_frozen_decisao_recommended")
    ...
    frozen_artifact = {
        **artifact,
        ...
        "frozen_approve_to_confirm_col": "exp014b_r4g_fast_frozen_approve_to_confirm",
        "frozen_block_to_confirm_col": "exp014b_r4g_fast_frozen_block_to_confirm",
        "frozen_intervention_pred_col": "exp014b_r4g_fast_frozen_intervention_pred",
        "frozen_block_pred_col": "exp014b_r4g_fast_frozen_block_pred",
        "frozen_intervention_metrics": frozen_intervention,
        "frozen_block_metrics": frozen_block,
    ...
    summary = {
        ...
        "frozen_intervention_metrics": frozen_intervention,
        "frozen_block_metrics": frozen_block,
        "n_approve_to_confirm": int(approve_to_confirm.sum()),
...
- APROVAR -> CONFIRMAR: `{int(approve_to_confirm.sum())}`
...
"""
    (out / "07_exp014b_r4g_fast_frozen_report.md").write_text(report, encoding="utf-8")
