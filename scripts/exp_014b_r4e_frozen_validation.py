# -*- coding: utf-8 -*-
"""
EXP-014B-R4E-FROZEN — Frozen validation do R4E.

Reaplica o artifact recomendado do EXP-014B-R4E sobre o baseline R4D-FROZEN
e valida replay exato:
  - métricas de intervenção;
  - métricas de BLOQUEAR;
  - predições recomendadas, se o arquivo EXP-014B-R4E/13_predictions_recommended.csv existir.

Saídas:
  resultados/experimentos/EXP-014B-R4E-FROZEN/
"""
...
EXPERIMENT = "EXP-014B-R4E-FROZEN"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--r4e-predictions", default=None)
...
def defaults() -> tuple[Path, Path, Path, Path]:
    root = Path.cwd()
    return (
        root / "resultados" / "experimentos" / "EXP-014B-R4D-FROZEN" / "06_predictions_frozen.csv",
        root / "resultados" / "experimentos" / "EXP-014B-R4E" / "12_policy_artifact_recommended.json",
        root / "resultados" / "experimentos" / "EXP-014B-R4E" / "13_predictions_recommended.csv",
...
def mask_from_description(df: pd.DataFrame, desc: str) -> pd.Series:
    prefixes = [
        "Mover CONFIRMAR para APROVAR R4E com ",
        "Mover BLOQUEAR para CONFIRMAR R4E com ",
        "Mover CONFIRMAR para BLOQUEAR R4E com ",
    ]
    prefix = None
    for p in prefixes:
...
def main() -> None:
    ...
    if not artifact_path.exists():
        raise FileNotFoundError(f"Artifact R4E não encontrado: {artifact_path}")
    ...
    ref_path = Path(args.r4e_predictions) if args.r4e_predictions else ref_default
    ...
    # Replay must follow original R4E phase order.
    confirm_to_approve = pd.Series(False, index=df.index)
    for rule in artifact.get("selected_confirm_to_approve_rules", []):
        confirm_to_approve |= base_action.eq("CONFIRMAR") & mask_from_description(df, str(rule["description"]))
    ...
    for rule in artifact.get("selected_block_to_confirm_rules", []):
        block_to_confirm |= action_after_a.eq("BLOQUEAR") & mask_from_description(df, str(rule["description"]))
    ...
    for rule in artifact.get("selected_confirm_to_block_rules", []):
        confirm_to_block |= action_after_b.eq("CONFIRMAR") & mask_from_description(df, str(rule["description"]))

    final_action = action_after_b.copy()
    final_action.loc[confirm_to_block] = "BLOQUEAR"

    df["exp014b_r4e_frozen_confirm_to_approve"] = confirm_to_approve.astype(int)
    df["exp014b_r4e_frozen_block_to_confirm"] = block_to_confirm.astype(int)
    df["exp014b_r4e_frozen_confirm_to_block"] = confirm_to_block.astype(int)
    df["r4e_frozen_decisao_recommended"] = final_action
    df["exp014b_r4e_frozen_intervention_pred"] = intervention_from_action(final_action)
    df["exp014b_r4e_frozen_block_pred"] = block_from_action(final_action)

    frozen_intervention = metrics(df[label_col], df["exp014b_r4e_frozen_intervention_pred"])
    frozen_block = metrics(df[label_col], df["exp014b_r4e_frozen_block_pred"])
    ...
    if ref_path.exists():
        ...
        src_action = artifact.get("final_action_col", "r4e_decisao_recommended")
        src_intervention = artifact.get("intervention_pred_col", "exp014b_r4e_intervention_pred")
        src_block = artifact.get("block_pred_col", "exp014b_r4e_block_pred")
        if src_action in ref.columns:
            mismatch |= actions(ref[src_action]).ne(actions(df["r4e_frozen_decisao_recommended"]))
        if src_intervention in ref.columns:
            mismatch |= ints(ref[src_intervention]).ne(ints(df["exp014b_r4e_frozen_intervention_pred"]))
        if src_block in ref.columns:
            mismatch |= ints(ref[src_block]).ne(ints(df["exp014b_r4e_frozen_block_pred"]))
        n_mismatch = int(mismatch.sum())
        cols = [c for c in [label_col, base_action_col, "r4e_frozen_decisao_recommended", "lgbm_r4_score", "score_final"] if c in df.columns]
        mismatch_df = df.loc[mismatch, cols].copy()

    status = "PASS_R4E_FROZEN_VALIDATED_REPLAY_OK" if intervention_ok and block_ok and n_mismatch == 0 else "FAIL_R4E_FROZEN_VALIDATION_MISMATCH"

    by_action = action_table(df, label_col, "r4e_frozen_decisao_recommended")
    ...
    validation = {
        ...
        "frozen_action_col": "r4e_frozen_decisao_recommended",
        "frozen_intervention_col": "exp014b_r4e_frozen_intervention_pred",
        "frozen_block_col": "exp014b_r4e_frozen_block_pred",
    ...
    frozen_artifact = {
        **artifact,
        ...
        "frozen_action_col": "r4e_frozen_decisao_recommended",
        "frozen_confirm_to_approve_col": "exp014b_r4e_frozen_confirm_to_approve",
        "frozen_block_to_confirm_col": "exp014b_r4e_frozen_block_to_confirm",
        "frozen_confirm_to_block_col": "exp014b_r4e_frozen_confirm_to_block",
        "frozen_intervention_pred_col": "exp014b_r4e_frozen_intervention_pred",
        "frozen_block_pred_col": "exp014b_r4e_frozen_block_pred",
        "frozen_intervention_metrics": frozen_intervention,
        "frozen_block_metrics": frozen_block,
    ...
    summary = {
        ...
        "frozen_intervention_metrics": frozen_intervention,
        "frozen_block_metrics": frozen_block,
        "n_confirm_to_approve": int(confirm_to_approve.sum()),
...
## Decisão sugerida
Se PASS, consolidar R4E-FROZEN como baseline global campeão FPR<1%.
A próxima rodada deve ajustar apenas BLOQUEAR -> CONFIRMAR sem mudar intervenção global.
"""
    (out / "07_exp014b_r4e_frozen_report.md").write_text(report, encoding="utf-8")
