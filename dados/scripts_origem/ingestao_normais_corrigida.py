...
========================================
POC Anomalia PIX — Ingestão Normais Trimestre V3.0 (LEAKAGE-FREE)

Alterações v3.0 (sobre v2.3):
  ══════════════════════════════════════════════════════════════
  PROBLEMA RESOLVIDO: Features trimestrais calculadas sobre
    TODO o trimestre (groupBy CPF) causavam leakage temporal.
    O modelo via o futuro no treino mas não em produção.

  SOLUÇÃO: Métricas trimestrais calculadas via ROLLING WINDOW
    com rangeBetween(-90 dias, -1 segundo) — exclui a tx atual
    e só usa transações ANTERIORES. Reproduz exatamente o que
    o Feature Store (Redis) terá em produção.

  FEATURES CORRIGIDAS (8 diretas):
    - qt_total_pix_trimestre      → count(*) rolling 90d antes
    - vl_mediana_pix_trimestre    → percentile_approx rolling
    - vl_desvio_padrao_pix_trimestre → stddev rolling
    - qt_aparelhos_distintos_trimestre → approx_count_distinct rolling
    - vl_latencia_rede_media_trimestre → avg rolling
    - vl_tempo_interacao_medio_trimestre → avg rolling
    - qt_pix_dia_maximo_trimestre → derivada de daily counts rolling
    - qt_intervalo_mediana/desvio → calculadas sobre intervalos rolling

  FEATURES DERIVADAS CORRIGIDAS AUTOMATICAMENTE (10):
    ratio_valor_mediana, diff_valor_mediana, ratio_valor_desvio_padrao,
    zscore_valor_aprox, ratio_intervalo_vs_mediana, diff_intervalo_vs_mediana,
    zscore_intervalo_aprox, ratio_latencia_cliente, diff_latencia_cliente,
    is_first_tx_trimestre — todas derivam das 8 acima no preprocessing.py

  NOTA: qt_pix_dia_maximo_trimestre não tem window function nativa
    eficiente. Usamos abordagem de self-join com daily_counts filtrado
    por data < data_tx_atual.

  Mantém TODAS as etapas e otimizações da v2.3.
  Mesma saída (mesmas colunas).
  ══════════════════════════════════════════════════════════════
"""
...
# Constante: 90 dias em segundos para rangeBetween
...
def create_spark_session():
    return (
        ...
        .appName("POC - PIX Anomalia Normais Trimestre V3.0 (Leakage-Free)")
        ...
        # Shuffle — aumentado para rolling windows pesadas
        .config("spark.sql.shuffle.partitions", "400")
        .config("spark.default.parallelism", "400")
        # Broadcast conservador
        .config("spark.sql.autoBroadcastJoinThreshold", "10485760")  # 10MB
        .config("spark.sql.broadcastTimeout", "600")
        # Timeout — aumentado para rolling windows
        .config("spark.network.timeout", "1800s")
        .config("spark.executor.heartbeatInterval", "60s")
        # Parquet
        .config("spark.sql.parquet.enableVectorizedReader", "true")
        .config("spark.sql.hive.convertMetastoreParquet", "false")
        # Memory overhead — aumentado para rolling windows
...
def main():
    ...
    output_table = "hmo_ml.tb_pix_anomalia_normais_trim_poc_v3"
    mbk_source_table = "landing_brb_oracle_mbk.aut"

    # Tabelas temporárias
    TMP_PIX = "hmo_ml.tmp_v30_pix_dedup"
    TMP_MOBILE = "hmo_ml.tmp_v30_mobile_dedup"
    TMP_BASE = "hmo_ml.tmp_v30_base_joined"
    TMP_DAILY = "hmo_ml.tmp_v30_daily_counts"

    t_start = time.time()

    print("=" * 80)
    print("PROCESSAMENTO UNIFICADO PIX + MOBILE V3.0 (LEAKAGE-FREE)")
    print("=" * 80)
    ...
    # =============================================================
    # ETAPA 1: CLIENTES (idêntico à v2.3)
    # =============================================================
    print("\n" + "=" * 60)
    ...
    # =============================================================
    # ETAPA 2: PIX BRUTO → TABELA TEMPORÁRIA (idêntico à v2.3)
    # =============================================================
    print("\n" + "=" * 60)
    ...
    # =============================================================
    # ETAPA 3: MOBILE MBK → TABELA TEMPORÁRIA (idêntico à v2.3)
    # =============================================================
    print("\n" + "=" * 60)
    ...
    # =============================================================
    # ETAPA 5: FEATURES ROLLING POR TRANSAÇÃO (LEAKAGE-FREE)
    # =============================================================
    # ══════════════════════════════════════════════════════════════
    # MUDANÇA PRINCIPAL v3.0:
    #   ANTES: groupBy("cd_cpf_pagador").agg(...) → 1 valor por CPF
    #          aplicado a TODAS as tx → LEAKAGE (tx vê o futuro)
    #
    #   AGORA: Window rolling com rangeBetween(-90d, -1s)
    #          Cada tx vê APENAS transações anteriores do mesmo CPF
    #          Reproduz exatamente o que o Redis terá em produção
    #
    # NOTA SOBRE PERFORMANCE:
    #   Rolling windows são mais pesadas que groupBy.
    #   Mitigações aplicadas:
    #     - shuffle.partitions aumentado para 400
    #     - memoryOverhead aumentado para 4GB
    #     - Materialização intermediária de daily_counts
    #     - qt_pix_dia_maximo via self-join (não há window nativa)
    # ══════════════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    print("ETAPA 5/6: FEATURES ROLLING (LEAKAGE-FREE)")
    print("=" * 60)
    ...
    # ─── Converter dt_pix para long (segundos) uma vez ───
    df_base = df_base.withColumn("dt_pix_long", F.col("dt_pix").cast("long"))

    # ─── Window principal: 90 dias ANTES, excluindo a tx atual ───
    # rangeBetween(-90*86400, -1) = de 90 dias atrás até 1 segundo antes
    w_rolling_90d = (
    ...
    print("    [5a] Calculando métricas rolling por transação...")

    df_features = (
        df_base
        # ── qt_total_pix_trimestre: transações anteriores nos últimos 90d ──
        .withColumn(
            "qt_total_pix_trimestre",
            F.count("cd_pix").over(w_rolling_90d)
        )
        # ── vl_mediana_pix_trimestre: mediana dos valores anteriores ──
        .withColumn(
            "vl_mediana_pix_trimestre",
            F.percentile_approx("vl_pix", 0.5).over(w_rolling_90d)
        )
        # ── vl_desvio_padrao_pix_trimestre: stddev dos valores anteriores ──
        .withColumn(
            "vl_desvio_padrao_pix_trimestre",
            F.stddev("vl_pix").over(w_rolling_90d)
        )
        # ── qt_aparelhos_distintos: approx_count_distinct não suporta window
        #    Usamos collect_set + size como workaround (ok para ~5000 tx/cpf) ──
        .withColumn(
            "qt_aparelhos_distintos_trimestre",
            F.size(F.collect_set("device_name").over(w_rolling_90d))
        )
        # ── vl_latencia_rede_media_trimestre: média de latência anterior ──
        .withColumn(
            "vl_latencia_rede_media_trimestre",
            F.avg("latencia_rede_ms").over(w_rolling_90d)
        )
        # ── vl_tempo_interacao_medio_trimestre: média de tempo interação ──
        .withColumn(
    ...
    print("    [5b] Calculando qt_pix_dia_maximo_trimestre rolling...")

    # ── qt_pix_dia_maximo_trimestre: MAX(daily_count) nos dias anteriores ──
    # Estratégia: materializar daily_counts, depois self-join filtrado por data
    df_daily_counts = (
        df_base
    ...
    df_daily_counts, _ = save_temp(df_daily_counts, spark, TMP_DAILY)

    # Para cada tx, pegar MAX(daily_count) onde data_daily < data_tx
    # e data_daily >= data_tx - 90 dias
    df_daily_max_per_tx = (
    ...
    print("    [5c] Calculando features de intervalo e primeiro envio...")

    # ── Intervalo entre transações (idêntico à v2.3) ──
    ...
    print("    [5d] Calculando mediana e desvio de intervalos rolling...")

    # ── Mediana e desvio de intervalos: rolling sobre intervalos anteriores ──
    # Precisamos do intervalo de transações ANTERIORES, não da tx atual
    w_rolling_intervalo = (
        Window.partitionBy("cd_cpf_pagador")
        .orderBy("dt_pix_long")
        .rangeBetween(-NINETY_DAYS_SECONDS, -1)
    ...
    elapsed_5 = round((time.time() - t5) / 60, 1)
    print(f"    ✅ Features rolling calculadas ({elapsed_5} min)")

    # =============================================================
    # ETAPA 6: SELEÇÃO FINAL + SAVE (idêntico à v2.3)
    # =============================================================
    print("\n" + "=" * 60)
    print("ETAPA 6/6: SELEÇÃO FINAL + SAVE")
    print("=" * 60)
    ...
    # ── VALIDAÇÃO DE LEAKAGE: amostra para conferência manual ──
    print("\n    --- VALIDAÇÃO LEAKAGE-FREE (amostra) ---")
    print("    Para um CPF com múltiplas tx, qt_total deve CRESCER ao longo do tempo:")
    df_check_sample = (
        df_check
        .groupBy("cd_cpf_pagador")
        .agg(F.count("*").alias("n_tx"))
        .filter(F.col("n_tx").between(5, 20))
        .limit(1)
    )
    ...
    if sample_cpf_rows:
        ...
        print("    (Nenhum CPF com 5-20 tx encontrado para amostra)")
    ...
    if CLEANUP_TEMP_TABLES:
        ...
        for t in [TMP_PIX, TMP_MOBILE, TMP_BASE, TMP_DAILY]:
    ...
    print("\n" + "=" * 80)
    print(f"CONCLUÍDO — V3.0 (LEAKAGE-FREE)")
