"""
train_lgbm.py v5.1 — LGBM Leakage-Free + Relatório Completo

Mudanças v5.1 vs v5.0:
  1. FIX: calibração usa cv=3 em vez de cv="prefit" (compatível com sklearn)
  2. FIX: artefatos reorganizados — só joblib em /backend/artefatos,
     todos os 10 CSVs/JSONs de relatório em /backend/modelos/resultado_treino_lgbm
  3. FIX: threshold sweep com step 0.005 (era 0.01)
  4. FIX: score distribution do train preserva todas as fraudes
  5. FIX: feature importance normalizada (% do total gain)
  6. REMOVIDO: EXTRA_FEATURES com gain ~0 que não contribuem pro LGBM
     (is_login_senha_flag, is_business_hours — mantidas no pipeline SE)
"""
...
log = logging.getLogger("train_lgbm_v5")
...
RESULT_DIR = PROJECT_ROOT / "backend" / "modelos" / "resultado_treino_lgbm"

INPUT_DATA = DADOS_DIR / "base_treino_final.csv"

# ── Produção: apenas o modelo serializado ──
MODEL_PATH = ARTEFACT_DIR / "model_lightgbm.joblib"

# ── Relatório: 10 artefatos de treino ──
METRICS_PATH = RESULT_DIR / "metricas_lgbm_v5.json"
...
# EXTRA: features com contribuição real no LGBM
# (is_login_senha_flag e is_business_hours removidas: gain ~0,
#  mantidas no pipeline SE/Engine mas não alimentam o modelo)
EXTRA_FEATURES = [
    "ratio_pix_renda",
    "vl_renda_cliente",
...
def evaluate_full(y_true: np.ndarray, y_prob: np.ndarray, threshold: float = 0.5) -> dict:
...
def print_split_summary(name: str, df_sub: pd.DataFrame, label_col: str = "is_fraud") -> str:
    n = len(df_sub)
    n_fraud = int(df_sub[label_col].sum())
    date_min = df_sub["event_datetime"].min()
    date_max = df_sub["event_datetime"].max()
    msg = (
        f"  {name}: {n:>7,} rows | {n_fraud:>4} fraudes ({n_fraud / max(n, 1) * 100:.2f}%) | "
        f"{date_min} → {date_max}"
    )
    print(msg)
    return msg
...
def main() -> None:
    ...
    print("=" * 72)
    print("  TREINO LightGBM v5.1 — Leakage-Free + Relatório Completo")
    print("=" * 72)
    ...
    if not INPUT_DATA.exists():
        ...
        tlog("   Rode primeiro: python scripts/fix_leakage_features.py")
        sys.exit(1)
    ...
    tlog(f"Fraudes: {total_fraud} | Normais: {total_normal} | Ratio: {df['is_fraud'].mean() * 100:.3f}%")
    ...
    for f in CORE_FEATURES + EXTRA_FEATURES:
    ...
    tlog(f"Features: {len(feature_cols)} total ({n_core} core + {n_extra} extra)")
    ...
    if nan_report:
        ...
        for feat, count in sorted(nan_report.items(), key=lambda x: -x[1])[:10]:
            ...
            tlog(f"  {feat}: {count:,} NaN ({count / total_rows * 100:.1f}%){fixed_tag}")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(tscv.split(X_dev)):
        ...
        tlog(f"  Treino: {len(X_tr):,} rows, {n_pos_tr} fraudes ({date_tr_min} → {date_tr_max})")
        tlog(f"  Val:    {len(X_va):,} rows, {n_pos_va} fraudes ({date_va_min} → {date_va_max})")
        ...
        if n_pos_va >= 2:
            ...
            best_iter = getattr(fold_model, "best_iteration_", fold_model.n_estimators)
        else:
    ...
    tlog(f"OOF: {oof_mask.sum():,} predições ({oof_mask.sum() / len(df_dev) * 100:.1f}% do dev)")
    ...
    tlog(f"✅ Modelo treinado — {n_features_used}/{len(feature_cols)} features com gain > 0")

    # ----------------------------------------------------------
    # 7. CALIBRAÇÃO (cv=3, isotonic — compatível com todas as versões sklearn)
    # ----------------------------------------------------------
    tlog("Calibrando probabilidades...")
    use_calibrated = False
    calibrated_model = None

    # Usar últimos 30% do dev como set de calibração
    ...
    if cal_fraud >= 10:
        try:
            # Treinar modelo leve no set de calibração para calibrar via CV
            ...
            use_calibrated = True
            tlog(f"✅ Calibração isotonic (cv=3) OK")
    ...
    # Train probabilities (para gap analysis)
    ...
    # Se calibração funcionou, buscar threshold calibrado também
    th_f1_cal = th_f1
    if use_calibrated:
    ...
    if use_calibrated:
        holdout_metrics_cal = evaluate_full(y_holdout.values, y_ho_prob_cal, th_f1_cal)
    ...
    fi_df = pd.DataFrame({
        ...
        "importance_gain_pct": np.round(importances_gain / max(total_gain, 1) * 100, 4),
    ...
    if fold_models:
        ...
        for fm_model in fold_models:
            avg_gain += fm_model.booster_.feature_importance(importance_type="gain")
    ...
    tlog("Top 15 features (gain):")
    for _, row in fi_df.head(15).iterrows():
        tag = " ⚠️FIX" if row["is_leakage_fixed"] else ""
        tlog(
            f"  #{int(row['rank']):2d} {row['feature']:45s} "
            f"gain={row['importance_gain']:12.1f} ({row['importance_gain_pct']:5.2f}%){tag}"
        )
    ...
    prec_arr, rec_arr, pr_thresholds = precision_recall_curve(y_holdout, y_ho_prob)
    pr_df = pd.DataFrame({
        "precision": prec_arr[:-1],
        "recall": rec_arr[:-1],
        "threshold": pr_thresholds,
    })
    # Amostrar PR curve (pode ter 10k+ pontos) para manter CSV razoável
    if len(pr_df) > 2000:
        step = len(pr_df) // 2000
    ...
    # Train: TODAS as fraudes + amostra de normais
    rng = np.random.RandomState(42)
    train_fraud_idx = np.where(y_dev.values == 1)[0]
    train_normal_idx = np.where(y_dev.values == 0)[0]
    train_normal_sample = rng.choice(
        train_normal_idx, size=min(20000, len(train_normal_idx)), replace=False
    )
    train_sample_idx = np.sort(np.concatenate([train_fraud_idx, train_normal_sample]))

    score_dist_train = pd.DataFrame({
        "split": "train",
        "is_fraud": y_dev.values[train_sample_idx],
        "score_raw": np.round(y_train_prob[train_sample_idx], 8),
        "score_calibrated": np.nan,  # Calibração não aplica ao train do modelo raw
    })
    ...
    for split_name, y_true, y_prob_arr in [
        ...
        stats = {
            ...
            "normal_p75": round(float(np.percentile(normal_scores, 75)), 8),
            "normal_p99": round(float(np.percentile(normal_scores, 99)), 6),
        }
        score_stats[split_name] = stats
        tlog(f"Score dist ({split_name}):")
        tlog(f"  Fraudes: mean={stats['fraud_mean']:.4f} med={stats['fraud_median']:.4f} "
             f"min={stats['fraud_min']:.6f} max={stats['fraud_max']:.6f}")
        tlog(f"  Normais: mean={stats['normal_mean']:.6f} med={stats['normal_median']:.6f} "
             f"max={stats['normal_max']:.6f} p99={stats['normal_p99']:.6f}")

    # ----------------------------------------------------------
    # 12. ERROR ANALYSIS (incluído no holdout_predictions)
    # ----------------------------------------------------------
    ...
    holdout_pred_df = pd.DataFrame({
        ...
        "error_type": np.where(
            ...
            np.where((y_holdout.values == 0) & (y_ho_pred == 1), "FP", "OK")
        ),
    })

    # Features de contexto para análise de erros
    analysis_features = [
    ...
    if "transaction_id" in df_holdout.columns:
        holdout_pred_df.insert(0, "transaction_id", df_holdout["transaction_id"].values)
    if "customer_id" in df_holdout.columns:
        holdout_pred_df.insert(1, "customer_id", df_holdout["customer_id"].values)
    if "event_datetime" in df_holdout.columns:
        holdout_pred_df.insert(2, "event_datetime", df_holdout["event_datetime"].values)
    ...
    if n_fn > 0:
        ...
        for _, row in fn_rows.iterrows():
            tlog(
                f"  score={row['score_raw']:.6f} vl_pix={row.get('vl_pix', '?'):>10} "
                f"burst={row.get('burst_30m_flag', '?')} tx30m={row.get('tx_count_prev_30m', '?')} "
    ...
    if n_fp > 0:
        tlog(f"FP (top 5 por score):")
        fp_rows = holdout_pred_df[holdout_pred_df["error_type"] == "FP"].nlargest(5, "score_raw")
    ...
    # ── PRODUÇÃO: só o joblib ──
    joblib.dump(final_model, MODEL_PATH)
    tlog(f"✅ PRODUÇÃO: {MODEL_PATH}")

    # ── RELATÓRIO: 10 arquivos ──
    # 1. metricas_lgbm_v5.json (master)
    # 2. cv_fold_metrics.json
    # 3. lgbm_features.json
    # 4. thresholds_config.json
    # 5. feature_importance.csv
    # 6. oof_predictions.csv
    # 7. holdout_predictions.csv
    # 8. score_distribution.csv
    # 9. threshold_sweep.csv
    # 10. training_log.txt
    ...
    features_config = {
        ...
        "removed_from_v50": ["is_login_senha_flag", "is_business_hours"],
    }
    ...
    thresholds_config = {
        ...
        "threshold_f1_calibrated": round(float(th_f1_cal), 6) if use_calibrated else None,
        ...
        "recommended_production": {
            ...
            "note": "Threshold de melhor F1 no holdout. Engine aplica cascade e vetos em cima.",
        },
    }
    with open(THRESHOLDS_PATH, "w", encoding="utf-8") as f:
        json.dump(thresholds_config, f, ensure_ascii=False, indent=2)

    # 5. feature_importance.csv — já salvo acima

    # 6. oof_predictions.csv — já salvo acima

    # 7. holdout_predictions.csv — já salvo acima

    # 8. score_distribution.csv — já salvo acima

    # 9. threshold_sweep.csv — já salvo acima

    # ----------------------------------------------------------
    # 14. MÉTRICAS MASTER (subsídio para relatório)
    # ----------------------------------------------------------
    elapsed = time.time() - t_start

    # Comparativo v4.1 vs v5.1
    comparison = {}
    for metric_name in ["roc_auc", "average_precision", "f1", "recall", "precision", "fn", "fp", "tp"]:
        v41_val = V41_METRICS.get(metric_name)
        v50_val = holdout_metrics_f1.get(metric_name)
        if v41_val is not None and v50_val is not None:
            if isinstance(v41_val, float) and isinstance(v50_val, float):
                delta = round(v50_val - v41_val, 6)
            else:
                delta = int(v50_val) - int(v41_val)
    ...
    # Leakage impact (inline, sem arquivo separado)
    leakage_impact = {}
    for feat in sorted(LEAKAGE_FIXED_FEATURES):
        ...
        leakage_impact[feat] = {
            "mean_all": round(float(vals.mean()), 4) if vals.notna().any() else None,
            "mean_fraud": round(float(fraud_vals.mean()), 4) if fraud_vals.notna().any() else None,
            "mean_normal": round(float(normal_vals.mean()), 4) if normal_vals.notna().any() else None,
    ...
    master_metrics = {
        "meta": {
            "version": "v5.1",
            "description": "LGBM leakage-free — calibração corrigida, artefatos reorganizados",
        ...
        "dataset": {
            ...
            "date_range_dev": f"{df_dev['event_datetime'].min()} → {df_dev['event_datetime'].max()}",
            "date_range_holdout": f"{df_holdout['event_datetime'].min()} → {df_holdout['event_datetime'].max()}",
        },
        "model": {
            ...
            "calibration_method": "isotonic_cv3" if use_calibrated else None,
        },
        "cross_validation": {
        ...
        "holdout": {
            ...
            "calibrated_at_best_f1": holdout_metrics_cal if use_calibrated else None,
        },
        "train_metrics": {
        ...
        "overfitting_analysis": {
            "gap_auc_train_holdout": round(
                (train_metrics.get("roc_auc") or 0) - (holdout_metrics_f1.get("roc_auc") or 0), 6
            ),
            "gap_f1_train_holdout": round(
                (train_metrics.get("f1") or 0) - (holdout_metrics_f1.get("f1") or 0), 4
            ),
        ...
        "comparison_v41_vs_v51": comparison,
        "score_distribution": score_stats,
        "roc_curve": roc_data,
        "leakage_impact": leakage_impact,
        "features": {
            "top_20_gain": fi_df.head(20)[
                ["rank", "feature", "importance_gain", "importance_gain_pct", "is_leakage_fixed"]
        ...
        "errors": {
            "fn_count": n_fn,
            "fp_count": n_fp,
            "fn_details": holdout_pred_df[holdout_pred_df["error_type"] == "FN"][
                ["score_raw"] + [f for f in analysis_features if f in holdout_pred_df.columns]
            ].to_dict(orient="records") if n_fn > 0 else [],
            "fp_details": holdout_pred_df[holdout_pred_df["error_type"] == "FP"].nlargest(10, "score_raw")[
                ["score_raw"] + [f for f in analysis_features if f in holdout_pred_df.columns]
            ].to_dict(orient="records") if n_fp > 0 else [],
        },
        "artefacts": {
            "production": {
    ...
    # 1. Master metrics (precisa ser o último a salvar pois contém tudo)
    ...
    # ----------------------------------------------------------
    print(f"\n{'=' * 72}")
    print("  RESULTADOS — LGBM v5.1 (Leakage-Free)")
    print(f"{'=' * 72}")

    m = holdout_metrics_f1
    print(f"\n  Dataset: {total_rows:,} tx ({total_fraud} fraudes)")
    print(f"  Dev: {len(df_dev):,} ({dev_fraud}F) | Holdout: {len(df_holdout):,} ({holdout_fraud}F)")
    print(f"  Features: {len(feature_cols)} ({n_features_used} com gain>0, {n_fixed} corrigidas)")
    print(f"  Calibração: {'✅ isotonic cv=3' if use_calibrated else '❌ falhou'}")
    ...
    if use_calibrated:
        ...
        print(f"    F1:       {mc['f1']:.4f}")
        print(f"    Recall:   {mc['recall']:.4f}  (TP={mc['tp']}, FN={mc['fn']})")
        print(f"    Precision:{mc['precision']:.4f}  (FP={mc['fp']})")

    print(f"\n  {'─' * 60}")
    print(f"  COMPARATIVO v4.1 → v5.1:")
    print(f"    {'Métrica':<22} {'v4.1 (leakage)':>16} {'v5.1 (clean)':>16} {'Delta':>10}")
    for name, vals in comparison.items():
        v41 = vals["v4.1"]
        v51 = vals["v5.1"]
        delta = vals["delta"]
        if isinstance(v41, float):
            print(f"    {name:<22} {v41:>16.4f} {v51:>16.4f} {delta:>+10.4f}")
        else:
            print(f"    {name:<22} {v41:>16} {v51:>16} {delta:>+10}")

    print(f"\n  {'─' * 60}")
    print(f"  OVERFITTING CHECK:")
    oa = master_metrics["overfitting_analysis"]
    print(f"    Gap AUC  (train-holdout): {oa['gap_auc_train_holdout']:+.6f}")
    print(f"    Gap F1   (train-holdout): {oa['gap_f1_train_holdout']:+.4f}")
    print(f"    Gap AP   (train-holdout): {oa['gap_ap_train_holdout']:+.6f}")
    ...
    print(f"\n{'=' * 72}")
    print("  ✅ LGBM v5.1 — Treino concluído")
    print(f"{'=' * 72}")
