...
## Fechamento da FASE 2 — Resultado Real

**Status:** ✅ Concluída — sucesso mínimo validado, sem promoção do LGBM v6.2  
**Decisão final:** manter o baseline pós-FASE 1, promover apenas a regra cirúrgica C1 e encerrar a busca por novas regras/modelos usando apenas os sinais atuais.

### 1. Resumo executivo

A FASE 2 tinha como objetivo reduzir os falsos negativos remanescentes por meio de retreinamento supervisionado, recalibração do motor de decisão e meta-modelagem, evitando novas regras manuais generalistas.

O resultado final foi positivo, mas mais limitado do que a meta forte inicialmente esperada. A fase mostrou que:

- o LGBM v6.2 orientado a recall pareceu promissor em avaliação model-only, mas não trouxe ganho líquido no `DecisionEngine` real;
- `first_receiver_flag` é um sinal forte, mas perigoso demais para virar regra hardcoded;
- regras amplas ou thresholds agressivos tendem a trocar FNs por FPs ou por TPs perdidos;
- ainda havia um único FN residual near-threshold recuperável com segurança;
- após a promoção da regra C1, os FNs restantes parecem próximos do limite dos dados atuais.

A FASE 2 termina com uma melhoria real e validada:

| Seed | Baseline pós-FASE 1 | Baseline final pós-C1 |
|---:|---|---|
| 42 | TP=346, FP=14, FN=9, F1=0,9678 | TP=347, FP=14, FN=8, F1≈0,9693 |
| 123 | TP=346, FP=12, FN=9, F1=0,9705 | TP=347, FP=12, FN=8, F1≈0,9720 |

Delta final da FASE 2:
...
| FP | +0 |
| TP perdido | 0 |
| F1 | melhora leve nos dois seeds |

### 2. EXP-005A — LGBM Recall-Oriented v6.2

O EXP-005A retreinou variantes do LightGBM com foco em recall. O melhor candidato model-only foi `LGBM_C_SPW_2_0X`.

Em avaliação model-only, o candidato reduziu FNs de forma expressiva, chegando a `FN=2` no sample avaliado, mas com aumento de FP. Esse resultado indicou que havia sinal supervisionado útil, mas que o modelo precisava ser testado dentro do motor real antes de qualquer promoção.

**Decisão:** candidato gerado, mas não promovido diretamente.

### 3. EXP-005B — Recalibração pós-LGBM v6.2

O EXP-005B testou o LGBM v6.2 dentro do `DecisionEngine` real, com diferentes configurações de threshold e guard rail.

O resultado E2E mostrou que o candidato não reduziu FN líquido no runtime. Os candidatos mantiveram `FN=9`, aumentaram FP e reduziram F1 em relação ao baseline.

A principal descoberta foi que o LGBM v6.2 recuperava alguns FNs, mas perdia outros TPs, gerando troca de fraudes capturadas sem ganho operacional líquido.

**Decisão:** rejeitar LGBM v6.2 para runtime. Manter como evidência diagnóstica, mas não promover artefatos nem thresholds do EXP-005B.

### 4. EXP-006 e EXP-006B — Cartografia de Erros e Contrafactuais

Os EXP-006 e EXP-006B foram criados para evitar novos E2E longos e passar a investigar os erros residuais de forma rápida, usando artefatos já existentes.

Esses experimentos confirmaram que:

- `first_receiver_flag` aparece em FNs recuperáveis, mas também domina FPs adicionados;
- portanto, `first_receiver_flag` deve ser tratado como feature/sinal, não como regra;
- os thresholds candidatos do LGBM v6.2 produziram métricas idênticas por seed, indicando que não diferenciavam a decisão final de forma útil;
- nenhuma regra ampla baseada em primeiro recebedor era segura para promoção.

**Decisão:** não criar regra generalista com `first_receiver_flag`; prosseguir apenas com hipóteses cirúrgicas e artifact-only.

### 5. EXP-006C/R2 — Regra de baixo valor com primeiro recebedor

O EXP-006C testou a regra `R2_LOW_VALUE_GRAY_FIRST_RECEIVER`, derivada dos contrafactuais do EXP-006B.

Apesar de parecer promissora nos casos movimentados, a regra falhou no quick-E2E:

| Seed | Delta FN | Delta FP | Decisão |
|---:|---:|---:|---|
| 42 | 0 | +1 | Rejeitar |
| 123 | 0 | +1 | Rejeitar |

A regra não recuperou nenhum FN e adicionou 1 FP nos dois seeds.

**Decisão:** rejeitar R2. Não promover nenhuma regra baseada em baixo valor + primeiro recebedor + LGBM em zona cinza.

### 6. EXP-006D — Censo dos FNs Residuais

O EXP-006D fez o censo dos 9 FNs residuais do baseline pós-FASE 1.

A análise mostrou:

| Classe | Quantidade aproximada |
|---|---:|
| `DATA_LIMITED_WEAK_ALL_MODULES` | 6 |
| `NEAR_THRESHOLD` | 1 |
| `MODULE_SIGNAL_WEAK` | 1 |
| `MIXED_OR_UNCLEAR` | 1 |

O achado central foi que apenas um FN parecia claramente recuperável com os sinais atuais. A maioria dos FNs tinha LGBM baixo, IF não extremo, SE=0, BEH=0 e score final distante do threshold.

**Decisão:** concentrar a busca em uma única hipótese cirúrgica near-threshold antes de concluir a fase.

### 7. EXP-006E e EXP-006F — C1 Near-Threshold

O EXP-006E desenhou a regra `C1_NEAR_THRESHOLD_REL_CURTO_FIRST_RECEIVER` em modo artifact-only. A regra mirava o único FN near-threshold identificado no censo.

A regra C1 foi definida como:
...
```json
{
  ...
  "score_final_min_inclusive": 58.0,
  "score_final_max_exclusive": 62.0,
  "se_score_max": 0.0,
  "beh_score_max": 0.0
}

A ação da regra é promover APROVAR → CONFIRMAR.

O EXP-006F validou a C1 no runtime/cached-runtime e depois em validação de transação real. O caso recuperado foi:

Campo	Valor
Transaction ID	E0000020820260205003505340630525
Valor	R$381,00
Relacionamento	6 meses
First receiver	1
Pix key random	0
LGBM raw	~0,063–0,068
Decisão original	APROVAR
Nova decisão	CONFIRMAR
...
Seed	TP	FP	FN	F1
42 baseline	346	14	9	0,9678
42 + C1	347	14	8	0,9693
123 baseline	346	12	9	0,9705
123 + C1	347	12	8	0,9720

Decisão: promover C1 como regra permanente, configurável e desligável.

8. EXP-007A — Meta-Learner Shadow

Após consolidar C1, foi executado o EXP-007A com baseline pós-C1.

O meta-learner shadow apresentou métricas globais fortes, com OOF AUC próximo de 0,996–0,998 e Average Precision próximo de 0,984–0,988. Porém, nenhum threshold gerou overlay seguro capaz de recuperar FN residual sem custo operacional em FP.

Resultado da decisão:

Status: SEM_CANDIDATO_SEGURO

O experimento confirmou que, embora os sinais atuais separem bem fraude e legítimos no geral, os 8 FNs restantes estão na fronteira difícil: valores baixos, SE=0, BEH=0, LGBM baixo/cinza e score final distante do threshold.

Decisão: não rodar EXP-007B. Não promover meta-learner. Não treinar novo LGBM com os mesmos sinais.

9. Decisão final da FASE 2

A FASE 2 deve ser encerrada com a seguinte decisão:
...
Rejeitado: novas regras generalistas com first_receiver.
Meta-learner: mantido como diagnóstico, sem candidato seguro.
Próxima etapa: FASE 3 — novas fontes de dados e reputação/grafo do recebedor.
10. Baseline consolidado pós-FASE 2

O novo baseline oficial passa a ser:

Seed	TP	FP	FN	Precision	Recall	F1
42	347	14	8	96,1219%	97,7465%	0,9693
123	347	12	8	96,6574%	97,7465%	0,9720

Em relação ao baseline pós-FASE 1, a FASE 2 reduziu FN de 9 para 8 sem aumentar FP.

11. Interpretação dos FNs remanescentes

Os 8 FNs restantes devem ser tratados como provavelmente próximos do limite dos sinais disponíveis.

A análise do censo e do meta-learner indica que a maior parte deles não possui convergência suficiente entre os módulos atuais:

LGBM baixo ou apenas cinza;
IF não extremo ou isolado;
SE=0;
BEH=0;
valores baixos ou moderados;
ausência de regra contextual forte;
score final distante do threshold de confirmação.

Portanto, insistir em novas regras manuais sobre os mesmos sinais tende a aumentar FP sem recuperar FN de forma robusta.

12. Mudanças permanentes recomendadas

Promover apenas as seguintes mudanças permanentes:

manter V1_GUARD_CONTEXTUAL do EXP-004-FINAL;
manter threshold_confirmar = 62;
manter threshold_bloquear = 95;
manter lgbm_guard_enabled = true;
manter lgbm_guard_threshold = 0.30;
manter EXP-003 residual desligado;
promover C1_NEAR_THRESHOLD_REL_CURTO_FIRST_RECEIVER com flag configurável;
manter LGBM v6.2 apenas como artefato candidato/rejeitado;
não promover meta-learner shadow.

Configuração conceitual final:
...
A FASE 2 se enquadra no cenário de sucesso mínimo parcial:

FN caiu de 9 para 8;
FP não aumentou;
F1 melhorou nos dois seeds;
a melhoria foi validada em seed 42 e seed 123;
o meta-learner não encontrou candidato seguro adicional;
os FNs remanescentes foram classificados como provavelmente dependentes de novos sinais.

A meta original de FN≤7 não foi atingida, mas o resultado é tecnicamente positivo porque a fase reduziu FN sem degradar a operação.

14. Próxima fase recomendada

A próxima fase não deve insistir em novos modelos com os mesmos sinais.

A recomendação é iniciar:

FASE 3 — Novas Fontes de Dados, Reputação de Recebedor e Grafo Transacional

Prioridades:

reputação histórica do recebedor;
recebedor recém-visto ou sem histórico;
quantidade de pagadores distintos por recebedor;
reincidência de recebedor em transações suspeitas;
grafo pagador-recebedor;
histórico de MED, contestação e devolução;
sinais de device/session/autenticação;
eventos recentes de segurança, troca de senha, troca de dispositivo ou alteração cadastral.

A hipótese para a próxima fase é que os 8 FNs residuais não são mais um problema de calibragem do motor atual, mas de ausência de informação contextual suficiente sobre recebedor, dispositivo, sessão e histórico pós-transação.
